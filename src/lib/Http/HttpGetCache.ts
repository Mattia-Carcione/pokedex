import { getCachedResponse } from "@/cache/cacheDb";
import { CachedAxiosResponse, CustomAxiosRequestConfig } from "@/cache/types/cacheTypes";
import { buildCacheKey } from "@/cache/utils/cacheKey";
...
export function setAxiosRequestCache(client: AxiosInstance): void {
    client.interceptors.request.use(async (config: InternalAxiosRequestConfig & CustomAxiosRequestConfig) => {
        ...
        try {
            ...
            const cachedResponse = await getCachedResponse<any>(cacheKey);
